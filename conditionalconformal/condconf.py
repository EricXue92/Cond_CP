import cvxpy as cp # convex programs
import numpy as np
...
class CondConf:
    def __init__(
            self, 
            score_fn : Callable, # conformity score
            Phi_fn : Callable, # finite-basis features used for conditional guarantees
            quantile_fn : Callable = None, # optional per-point target quantile (for adaptive targets).
            infinite_params : dict = {}, # kernel settings if you do RKHS
            seed : int = 0
        ):
    ...
    def setup_problem(
        ...
        # Prepares all calibration-side quantities (scores, features)
        # and builds a parametric CVXPY problem
        self.x_calib = x_calib
        self.y_calib = y_calib

        phi_calib = self.Phi_fn(x_calib) # compute Φ on the calibration set
        ...
        if r < len(s): # if not full rank, reduce to full rank
       ...
       # Build a parametric CVX problem for the augmented LP (calib + 1 test row).
    ...
    def predict_naive(
        ...
        if self.infinite_params.get('kernel', FUNCTION_DEFAULTS['kernel']):
            ...
            S = self.scores_calib.astype(float).reshape(-1, 1)
            Phi = self.phi_calib.astype(float)
            zeros = np.zeros((Phi.shape[1],))
            ...
            # res = linprog(-1 * S, A_eq=Phi.T, b_eq=zeros, bounds=bounds, method='highs')
            res = linprog(-S.ravel(), A_eq=Phi.T, b_eq=zeros, bounds=bounds, method="highs")
