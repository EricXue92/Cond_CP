...
# Finite-basis conditional thresholds
def conditional_thresholds(x_cal, scores_cal, x_test, alpha):
    """
    ...
    return x_test @ beta     # shape (n_test, )
...
def classification_prediction_sets(x_cal, y_cal, probs_cal, x_test, probs_test, alpha=0.1, exact=True):
    """
    ...
    t_cond = conditional_thresholds(x_cal, scores_cal, x_test, alpha)
...
def compute_conditional_coverage(x_test, scores_test, alpha):
    x_test = np.asarray(x_test, dtype=float)
    scores_test = np.asarray(scores_test, dtype=float).ravel()
    coveragesCond = np.zeros(len(x_test), dtype=bool)
...
# #   logitsCal, logitsTest  -> from your model
# #   yCal                    -> true labels of calibration set
# #   XCal, XTest             -> Phi(x) features
# # Convert logits to probabilities:
# probsCal  = softmax(logitsCal)   # (n_cal, K)
# probsTest = softmax(logitsTest)  # (n_test, K)
#
# out = classification_prediction_sets(
#     XCal, yCal, probsCal, XTest, probsTest, alpha=0.1
# )
#
# # Access results:
# split_sets  = out['split']['sets']     # list of arrays of class indices
# split_label = out['split']['labels']   # argmax point labels
# cond_sets   = out['cond']['sets']
# cond_label  = out['cond']['labels']
#
# t_split     = out['thresholds']['split']  # scalar
# t_cond      = out['thresholds']['cond']   # per-sample thresholds