...
def prepare_features_with_metadata(config, features_path=None):
...
def load_features_by_name(dataset_name, features_path=None):
    config = DATASET_CONFIG.get(dataset_name)
    if not config:
        raise ValueError(f"Unknown dataset: {dataset_name}")
    return prepare_features_with_metadata(config, features_path)
...
def save_and_plot(coverages_split, coverages_cond, metadata, test_idx, dataset_name, alpha):
    ...
    avail_cols = [col for col in group_cols if col in metadata.columns]
    if not avail_cols:
...
def main():
    ...
    features, logits, labels, metadata, train_idx, calib_idx, test_idx = load_features_by_name(
